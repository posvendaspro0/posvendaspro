  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter((claim) => {
        ...
        const responsible = (
          claim._complementary?.responsible ||
          claim.assigned_to ||
          ""
        ).toLowerCase();
        const product = (
          claim._complementary?.productSku ||
          claim.item_id ||
          ""
        ).toLowerCase();
    ...
    result.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case "responsible":
          aValue = (
            a._complementary?.responsible ||
            a.assigned_to ||
            ""
          ).toLowerCase();
          bValue = (
            b._complementary?.responsible ||
            b.assigned_to ||
            ""
          ).toLowerCase();
          break;
                      ...
                      >
                        <PopoverTrigger asChild>
                          ...
                          >
                            <CalendarIcon className="mr-2 h-4 w-4" />
                            {customDateTo
                              ? format(customDateTo, "dd 'de' MMMM 'de' yyyy", {
                                  locale: ptBR,
                                })
                              : "Selecione uma data"}
                          </Button>
        ...
        <Card className="border-orange-200 bg-orange-50">
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-orange-700 text-center max-w-md mb-4">
              Nenhuma reclamação corresponde aos filtros aplicados. Tente
              ...
              <TableBody>
                {paginatedClaims.map((claim) => {
                  ...
                  if (claim.date_created) {
                    ...
                    const dataResolucao = claim.date_closed
                      ? new Date(claim.date_closed)
                      : new Date();
                    const diffMs =
                      dataResolucao.getTime() - dataAbertura.getTime();
